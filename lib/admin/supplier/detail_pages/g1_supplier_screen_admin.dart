...
class G1SupplierScreenAdmin extends StatefulWidget {
  const G1SupplierScreenAdmin({super.key});

  @override
  State<StatefulWidget> createState() => _G1SupplierScreenAdminState();
}

class _G1SupplierScreenAdminState extends State<G1SupplierScreenAdmin> {
                                      ...
                                      ElevatedButton(
                                        onPressed: () async {
                                          ...
                                          if (_formKey.currentState!.validate()) {
                                            ...
                                            if (action == "update") {
                                              // Update the product
                                              await _g1Supplier.doc(documentSnapshot!.id).set({
                                                "namaPart": namaPart,
                                                "kodePart": kodePart,
                                                "jenisPart": jenisPart,
                                                "namaSupplier": namaSupplier,
  ...
  Future<void> _deleteProduct(String productId) async {
    await _g1Supplier.doc(productId).delete();
    ...
    return Scaffold(
                            ...
                            onPressed: () {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  ...
                                  content: Text(
                                    "GESITS G1 Supplier Parts was exported to Download folder!",
                                    style: TextStyle(
                                      color: Colors.white,
                                    ),
                                  ),
      ...
      body: RefreshIndicator(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: _g1Supplier.orderBy("namaPart", descending: false).snapshots(),
...
generateCsv() async {
  ...
  if (kIsWeb) {
    ...
    final File file = await (File("${generalDownloadDir.path}/G1_Supplier_Parts_$formattedDate.csv").create());
