...
class RayaSupplierScreenAdmin extends StatefulWidget {
  const RayaSupplierScreenAdmin({super.key});

  @override
  State<StatefulWidget> createState() => _RayaSupplierScreenAdminState();
}

class _RayaSupplierScreenAdminState extends State<RayaSupplierScreenAdmin> {
                                      ...
                                      ElevatedButton(
                                        onPressed: () async {
                                          ...
                                          if (_formKey.currentState!.validate()) {
                                            ...
                                            if (action == "update") {
                                              // Update the product
                                              await _rayaSupplier.doc(documentSnapshot!.id).set({
                                                "namaPart": namaPart,
                                                "kodePart": kodePart,
                                                "jenisPart": jenisPart,
                                                "namaSupplier": namaSupplier,
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: _rayaSupplier.orderBy("namaPart", descending: false).snapshots(),
...
generateCsv() async {
  ...
  if (kIsWeb) {
    ...
    final anchor = html.document.createElement("a") as html.AnchorElement
      ..href = url
      ..style.display = "none"
      ..download = "Raya_Supplier_Parts_$formattedDate.csv";
    html.document.body!.children.add(anchor);
    anchor.click();
    html.Url.revokeObjectUrl(url);
  } else if (Platform.isAndroid) {
    Directory generalDownloadDir = Directory("storage/emulated/0/Download");
    final File file = await (File("${generalDownloadDir.path}/Raya_Supplier_Parts_$formattedDate.csv").create());
