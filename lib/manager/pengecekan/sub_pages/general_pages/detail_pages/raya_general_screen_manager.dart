...
class RayaGeneralScreenManager extends StatefulWidget {
  const RayaGeneralScreenManager({super.key});

  @override
  State<StatefulWidget> createState() => _RayaGeneralScreenManagerState();
}

class _RayaGeneralScreenManagerState extends State<RayaGeneralScreenManager> {
  // Create a timestamp
  ...
  Future<DocumentSnapshot> fetchDataById(String documentId) async {
    DocumentSnapshot documentSnapshot =
        await FirebaseFirestore.instance.collection('raya_general_parts').doc(documentId).get();
                                      ...
                                      actions: [
                                        ElevatedButton(
                                          onPressed: () async {
                                            ...
                                            await _rayaGeneralParts.doc(documentSnapshot.id).set({
                                              "namaPart": namaPart,
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: _rayaGeneralParts.orderBy("tanggalPengecekan", descending: true).snapshots(),
...
generateCsv() async {
  ...
  if (kIsWeb) {
    ...
    final anchor = html.document.createElement("a") as html.AnchorElement
      ..href = url
      ..style.display = "none"
      ..download = "Raya_General_Parts_$formattedDate.csv";
    html.document.body!.children.add(anchor);
    anchor.click();
    html.Url.revokeObjectUrl(url);
  } else if (Platform.isAndroid) {
    Directory generalDownloadDir = Directory("storage/emulated/0/Download");
    final File file = await (File("${generalDownloadDir.path}/Raya_General_Parts_$formattedDate.csv").create());
