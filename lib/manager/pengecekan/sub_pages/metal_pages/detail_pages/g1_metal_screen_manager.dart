...
class G1MetalScreenManager extends StatefulWidget {
  const G1MetalScreenManager({super.key});

  @override
  State<StatefulWidget> createState() => _G1MetalScreenManagerState();
}

class _G1MetalScreenManagerState extends State<G1MetalScreenManager> {
  // Create a timestamp
  ...
  Future<DocumentSnapshot> fetchDataById(String documentId) async {
    DocumentSnapshot documentSnapshot =
        await FirebaseFirestore.instance.collection('g1_metal_parts').doc(documentId).get();
  ...
  final CollectionReference _g1MetalParts = FirebaseFirestore.instance.collection("g1_metal_parts");
                                      ...
                                      actions: [
                                        ElevatedButton(
                                          onPressed: () async {
                                            ...
                                            await _g1MetalParts.doc(documentSnapshot.id).set({
                                              "namaPart": namaPart,
    ...
    return Scaffold(
                            ...
                            onPressed: () {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  ...
                                  content: Text(
                                    "GESITS G1 Metal Parts was exported to Download folder!",
                                    style: TextStyle(
                                      color: Colors.white,
                                    ),
                                  ),
      ...
      body: RefreshIndicator(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: _g1MetalParts.orderBy("tanggalPengecekan", descending: true).snapshots(),
...
generateCsv() async {
  ...
  if (kIsWeb) {
    ...
    final anchor = html.document.createElement("a") as html.AnchorElement
      ..href = url
      ..style.display = "none"
      ..download = "G1_Metal_Parts_$formattedDate.csv";
    html.document.body!.children.add(anchor);
    anchor.click();
    html.Url.revokeObjectUrl(url);
  } else if (Platform.isAndroid) {
    Directory generalDownloadDir = Directory("storage/emulated/0/Download");
    final File file = await (File("${generalDownloadDir.path}/G1_Metal_Parts_$formattedDate.csv").create());
