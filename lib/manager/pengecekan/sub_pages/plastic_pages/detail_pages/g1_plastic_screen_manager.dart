...
class G1PlasticScreenManager extends StatefulWidget {
  const G1PlasticScreenManager({super.key});

  @override
  State<StatefulWidget> createState() => _G1PlasticScreenManagerState();
}

class _G1PlasticScreenManagerState extends State<G1PlasticScreenManager> {
  // Create a timestamp
  ...
  Future<DocumentSnapshot> fetchDataById(String documentId) async {
    DocumentSnapshot documentSnapshot =
        await FirebaseFirestore.instance.collection('g1_plastic_parts').doc(documentId).get();
                                      ...
                                      actions: [
                                        ElevatedButton(
                                          onPressed: () async {
                                            ...
                                            await _g1PlasticParts.doc(documentSnapshot.id).set({
                                              "namaPart": namaPart,
    ...
    return Scaffold(
                            ...
                            onPressed: () {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  ...
                                  content: Text(
                                    "GESITS G1 Plastic Parts was exported to Download folder!",
                                    style: TextStyle(
                                      color: Colors.white,
                                    ),
                                  ),
      ...
      body: RefreshIndicator(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: _g1PlasticParts.orderBy("tanggalPengecekan", descending: true).snapshots(),
...
generateCsv() async {
  ...
  if (kIsWeb) {
    ...
    final anchor = html.document.createElement("a") as html.AnchorElement
      ..href = url
      ..style.display = "none"
      ..download = "G1_Plastic_Parts_$formattedDate.csv";
    html.document.body!.children.add(anchor);
    anchor.click();
    html.Url.revokeObjectUrl(url);
  } else if (Platform.isAndroid) {
    Directory generalDownloadDir = Directory("storage/emulated/0/Download");
    final File file = await (File("${generalDownloadDir.path}/G1_Plastic_Parts_$formattedDate.csv").create());
